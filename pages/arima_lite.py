...
def main():
    ...
    if st.button('预测'):
        ...
        data_train = data.iloc[int(train):int(train) + int(num)].values.reshape(-1, 1)
        ...
        predictions = fitted_model.get_prediction(start=1, end=int(num), dynamic=False)
        ...
        # 显示动态图表
        y_pred = np.array(predicted_values).reshape(-1, 1)
        data_test = np.array(data_train).reshape(-1, 1)
