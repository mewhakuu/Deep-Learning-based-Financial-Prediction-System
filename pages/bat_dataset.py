...
# Define custom theme in config.toml (not shown here)
# [theme]
# primaryColor = "#E694FF"
# backgroundColor = "#00172B"
# ...

# Your DSARF model code (not shown here)
...
class GatedTransition(nn.Module):
    ...
    def forward(self, z_t_1):
        ...
        给定与时间对应的潜伏z_{t-1}
        步骤t-1，我们返回参数化（对角）高斯分布p（zt | z{t-1}）的平均值和标度向量
        z是L*Batch*z_dim
        """
...
class StateTransition(nn.Module):
    ...
    def forward(self, s_t_1, z_t_1):
        ...
        给定潜在的s_{t-1}，我们返回参数化类别分布p（s_t|s_{t-1}）的概率
        """
        if z_t_1 is None:
...
class Combiner(nn.Module):
    ...
    参数化q（z_t|z_{t-1}，x_{t:t}），这是指南的基本构建块（即变分分布）。
    对x_{t:t}的依赖是通过RNN的隐藏状态实现的（请参阅下面的pytorch模块“RNN”）
    """
    ...
    def forward(self, z_t_1, h_rnn):
        ...
        给定特定时间步长t-1处的潜伏z以及RNN h（x{t:t}）的隐藏状态，我们返回参数化（对角）高斯分布q（z_t|z{t-1}，y_{t:t}）
        """
...
class DSARF(nn.Module):
    ...
    class DSARF_(nn.Module):
        ...
        """#获得预测值
        def get_predict(self, data, steps = None):
            if steps is None:
        ...
        def plot_predict(self, data, steps = None, path = './plots/'):
            ...
            for j in range(0 , len(y_pred), max(len(y_pred)//4, 1)):
                ...
                save_path = path + "prediction_%d.png" %j
                plt.close()
                return save_path
...
def main():
    ...
    # 用户输入数据
    D_ = st.text_input('请输入D(默认为102)值：', '102')
    factor_dim_ = st.text_input('请输入factor_dim值(默认为5)：','5')
    S_ = st.text_input('请输入S(默认为2)值：','2')
    batch_size_ = st.text_input('请输入batch_size(默认值为1)值：','1')
    # Add interactive widgets
    st.sidebar.header("选择模型集体参数")
    num_epochs = st.sidebar.slider("训练轮数", 0,50,100)
    ...
    if st.button('预测'):
        ...
        # 模型预测
        dsarf = DSARF(int(D_), factor_dim = int(factor_dim_), L=[1,2], S=2, batch_size=int(batch_size_))
