...
@Time ： 2024/5/13 下午5:09
@Author ： mewhaku
@File ：page_wuliangye.py
@IDE ：PyCharm
"""
...
def main():
    ...
    # 用户输入数据
    data = st.text_input('请输入预测第个数据值：','1')#14
    ...
    if st.button('预测'):
        ...
        #数据分割
        path = file_path
        data = [pd.read_csv(path).values[int(train):int(train) + int(num)].reshape(-1, 1),
                pd.read_csv(path).values[int(test):int(test) + int(num)].reshape(-1, 1)]
        data_train = data[:1]
        data_test = data[1:]
        # 模型预测
        dsarf = DSARF(int(D_), factor_dim = int(factor_dim_), L=[1], S=int(S_), batch_size=int(batch_size_))
