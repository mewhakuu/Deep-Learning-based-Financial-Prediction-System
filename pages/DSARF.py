...
def main():
    ...
    # 用户输入数据
    num = st.text_input('请输入预测的数据(默认为14)：','14')
    ...
    if st.button('预测'):
        ...
        #数据分割
        path = file_path
        data = [pd.read_csv(path, sep=';', header=8).values[100:200, int(num)].reshape(-1, 1),
                pd.read_csv(path, sep=';', header=8).values[0:100, int(num)].reshape(-1, 1)]
        data_train = data[:1]
        data_test = data[1:]
